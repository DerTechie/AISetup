...
> **Status:** Phase 1 (routing gateway) complete; Phase 2 (triage advisor) built and unit-tested — live verification on the Mac gateway + Hermes pending. See the [design spec](docs/superpowers/specs/2026-05-20-hybrid-ai-routing-design.md) and the [decision journal](docs/journal/).
...
1. **Done.** Routing in place: Mac prep + Ollama (`pmset`, `OLLAMA_HOST`, pull model); LiteLLM gateway with `local` + `deep` routes, €100/mo budget cap, token cap, context fallback, dashboard; wire Hermes (default local, `/model deep` → cloud).
2. **Built; live verification pending.** Triage advisor skill (recommend / local-judge) — see [runbook § Triage advisor](docs/runbook.md#triage-advisor-phase-2).
